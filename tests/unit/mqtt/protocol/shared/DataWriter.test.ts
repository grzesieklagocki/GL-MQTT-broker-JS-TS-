...
describe("DataWriter", () => {
  ...
  describe("toUint8Array()", () => {
    it("returns Uint8Array with 1 element [0] when capacity is 1 and no bytes has been written", () => {
      const writer = new DataWriter(1);

      expect(writer.toUint8Array()).toStrictEqual(Uint8Array.from([0]));
    });

    it("returns Uint8Array with 1 element [160] when capacity is 1 and 1 byte has been written", () => {
      const writer = new DataWriter(1);
      const array = Uint8Array.from([160]);

      writer.write(array);

      expect(writer.toUint8Array()).toStrictEqual(array);
    });

    it("returns Uint8Array with 2 elements [160, 0] when capacity is 2 and 1 byte has been written", () => {
      const writer = new DataWriter(2);
      const array = Uint8Array.from([160]);

      writer.write(array);

      expect(writer.toUint8Array()).toStrictEqual(Uint8Array.from([160, 0]));
    });
  });
});
