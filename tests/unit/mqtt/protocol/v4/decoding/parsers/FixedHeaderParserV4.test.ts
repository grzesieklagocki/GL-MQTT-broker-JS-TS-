  ...
  describe("parse", () => {
    it("takes only Fixed Header bytes from reader", () => {
      [
        ...
        { packet: [0b0011_0000, 0x03, 0x00, 0x04, 0x54] }, // PUBLISH packet with remaining length 3
        {
          // SUBSCRIBE packet with remaining length 7
          packet: [0b1000_0010, 0x07, 0x00, 0x01, 0x00, 0x03, 0x61, 0x2f, 0x62],
        },
      ].forEach(({ packet }) => {
        const readerMock = createFixedHeaderReaderV4Mock(packet);
        const parser = new FixedHeaderParserV4();

        const fixedHeader = parser.parse(readerMock);

        expect(fixedHeader).not.toBeNull();
        expect(readerMock.readOneByteInteger).toHaveBeenCalledTimes(2);
      });
    });
    ...
    describe("Packet Type", () => {
      it("parses all supported Packet Types (MQTT 3.1.1)", () => {
        [
          ...
          expect(readerMock.readOneByteInteger).toBeCalledTimes(2); // first byte + Remaining Length on one byte
          expect(readerMock.readTwoByteInteger).not.toBeCalled(); // not used in Fixed Header parsing
          expect(readerMock.readBytes).not.toBeCalled(); // not used in Fixed Header parsing
          expect(readerMock.readString).not.toBeCalled(); // not used in Fixed Header parsing
          expect(readerMock.readBinaryData).not.toBeCalled(); // not used in Fixed Header parsing
        });
      });
      ...
      it("throws when Packet Type is reserved (MQTT 3.1.1)", () => {
        ...
        [[0b0000_0000], [0b1111_0000]].forEach((packet) => {
          ...
          expect(() => parser.parse(readerMock)).toThrow(/Packet Type/);

          expect(readerMock.readOneByteInteger).toHaveBeenCalledOnce(); // should throw after reading first byte
          expect(readerMock.readTwoByteInteger).not.toBeCalled(); // not used in Fixed Header parsing
          expect(readerMock.readBytes).not.toBeCalled(); // not used in Fixed Header parsing
          expect(readerMock.readString).not.toBeCalled(); // not used in Fixed Header parsing
          expect(readerMock.readBinaryData).not.toBeCalled(); // not used in Fixed Header parsing
      ...
      it("throws when Flags are invalid for given Packet Type (for all packet types)", () => {
        [
          ...
          invalidFlags.forEach((invalidFlag) => {
            ...
            const readerMock = createFixedHeaderReaderV4Mock([firstByte]);
            const parser = new FixedHeaderParserV4();
    ...
    describe("Remaining Length", () => {
      it("correctly parses Variable Byte Integer encodings (1..4 bytes)", () => {
        [
          ...
          expect(readerMock.readOneByteInteger).toBeCalledTimes(
            input.length + 1
          ); // first byte + Remaining Length byte(s)
          expect(readerMock.readTwoByteInteger).not.toBeCalled(); // not used in Fixed Header parsing
          expect(readerMock.readBytes).not.toBeCalled(); // not used in Fixed Header parsing
          expect(readerMock.readString).not.toBeCalled(); // not used in Fixed Header parsing
          expect(readerMock.readBinaryData).not.toBeCalled(); // not used in Fixed Header parsing
      ...
      it("throws for invalid Variable Byte Integer encodings", () => {
        [
          ...
          expect(() => parser.parse(readerMock)).toThrow(/Malformed/);

          expect(readerMock.readOneByteInteger).toBeCalledTimes(5); // first byte + remaining length (max 4 bytes)
          expect(readerMock.readTwoByteInteger).not.toBeCalled(); // not used in Fixed Header parsing
          expect(readerMock.readBytes).not.toBeCalled(); // not used in Fixed Header parsing
          expect(readerMock.readString).not.toBeCalled(); // not used in Fixed Header parsing
          expect(readerMock.readBinaryData).not.toBeCalled(); // not used in Fixed Header parsing
      ...
      it("throws when Remaining Length is invalid (for all packet types)", () => {
        ...
        invalidCases.forEach(([firstByte, invalidLengths]) => {
          invalidLengths.forEach((invalidLength) => {
            ...
            expect(() => parser.parse(readerMock)).toThrow(/Remaining Length/);
    ...
    describe("Chunked Inputs", () => {
      it("returns null when not enough data to parse Fixed Header", () => {
        [
          ...
          expect(readerMock.readOneByteInteger).toBeCalledTimes(packet.length); // all provided bytes
          expect(readerMock.readTwoByteInteger).not.toBeCalled(); // not used in Fixed Header parsing
          expect(readerMock.readBytes).not.toBeCalled(); // not used in Fixed Header parsing
          expect(readerMock.readString).not.toBeCalled(); // not used in Fixed Header parsing
          expect(readerMock.readBinaryData).not.toBeCalled(); // not used in Fixed Header parsing
    ...
    describe("Multiple Fixed Headers", () => {
      it("parses multiple Fixed Headers using one parser (from multiple readers)", () => {
        [
          ...
          const reader1 = createFixedHeaderReaderV4Mock(chunks[0]);
          const reader2 = createFixedHeaderReaderV4Mock(chunks[1]);
...
function testMultipleChunksParsing(chunks: number[][], expected: FixedHeader) {
  ...
  for (let i = 0; i < lastIndex; i++) {
    ...
    const readerMock = createFixedHeaderReaderV4Mock(chunk);
    const fixedHeader = parser.parse(readerMock);

    expect(fixedHeader).toBeNull(); // Not enough data yet

    expect(readerMock.readOneByteInteger).toBeCalledTimes(chunk.length); // all provided bytes
    expect(readerMock.readTwoByteInteger).not.toBeCalled(); // not used in Fixed Header parsing
    expect(readerMock.readBytes).not.toBeCalled(); // not used in Fixed Header parsing
    expect(readerMock.readString).not.toBeCalled(); // not used in Fixed Header parsing
    expect(readerMock.readBinaryData).not.toBeCalled(); // not used in Fixed Header parsing
  ...
  const readerMock = createFixedHeaderReaderV4Mock(lastChunk);
  const fixedHeader = parser.parse(readerMock);

  expect(fixedHeader).toStrictEqual(expected);

  expect(readerMock.readOneByteInteger).toBeCalledTimes(lastChunk.length); // all provided bytes
  expect(readerMock.readTwoByteInteger).not.toBeCalled(); // not used in Fixed Header parsing
  expect(readerMock.readBytes).not.toBeCalled(); // not used in Fixed Header parsing
  expect(readerMock.readString).not.toBeCalled(); // not used in Fixed Header parsing
  expect(readerMock.readBinaryData).not.toBeCalled(); // not used in Fixed Header parsing
